...
public class TransformEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        Vector3 posFeet = UnitConversions.MetersToFeet(posMeters); // Utils.MathConversion.MetersToFeet(posMeters);
        Vector3 rot = m_LocalRotation.quaternionValue.eulerAngles;
        Vector3 scale = m_LocalScale.vector3Value;

        EditorGUILayout.BeginHorizontal();
        {
            if (DrawButton("R", "Reset Position", IsResetPositionValid(pos), 20f))
            {
                pos = Vector3.zero;
            }
            EditorGUILayout.BeginVertical();
            {
                ...
                posUnityUnits = EditorGUILayout.Vector3Field("Local Position (m)", previousUnityUnits);
                ...
                {
                    pos = UnitConversions.FeetToMetres(posMeters); // Utils.MathConversion.FeetToMetres(posFeet);
                }
            }
        ...
        EditorGUILayout.BeginHorizontal();
        {
            if (DrawButton("R", "Reset Scale", IsResetScaleValid(scale), 20f))
            {
                scale = Vector3.one;
            }
